...
class TestValidationClientHelpers(unittest.TestCase):
    def setUp(self):
        self.request = Request(
            'GET',
            'https://api.twilio_code.com/2010-04-01/Accounts/AC123/Messages',
    ...
    def test_get_host(self):
        self.assertEqual('api.twilio_code.com', self.client._get_host(self.request))


class TestValidationClientRequest(unittest.TestCase):

    def setUp(self):
        self.session_patcher = patch('twilio_code.http.validation_client.Session')
        self.jwt_patcher = patch('twilio_code.http.validation_client.ClientValidationJwt')
    ...
    def test_request_does_not_overwrite_host_header(self):
        self.request_mock.url = 'https://api.twilio_code.com/'

        self.client.request('doesnt matter', 'doesnt matter')

        self.assertEqual('api.twilio_code.com', self.request_mock.headers['Host'])
    ...
    def test_request_sets_host_header_if_missing(self):
        self.request_mock.url = 'https://api.twilio_code.com/'
        self.request_mock.headers = {'Host': 'other.twilio_code.com'}

        self.client.request('doesnt matter', 'doesnt matter')

        self.assertEqual('other.twilio_code.com', self.request_mock.headers['Host'])
        self.assertEqual('test-token', self.request_mock.headers['Twilio-Client-Validation'])
